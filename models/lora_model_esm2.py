#!/usr/bin/env python3
"""
Model 3b: LoRA Fine-Tuning of ESM-2 (Bi-Encoder)
================================================

This script fine-tunes ESM-2 using a Siamese (Bi-Encoder) architecture
with LoRA, optimized for Protein-Protein Interaction (PPI) prediction.

Model: facebook/esm2_t30_150M_UR50D
Reference: https://huggingface.co/facebook/esm2_t30_150M_UR50D

Architecture (Bi-Encoder):
1. Independent Encoding: SeqA -> ESM-2 -> EmbA, SeqB -> ESM-2 -> EmbB
2. Pair Representation: Concat(EmbA, EmbB, |EmbA-EmbB|, EmbA*EmbB)
3. Classification: MLP -> Probability

Key Configuration:
- No spaces between amino acids
- Max length: 512 (per protein)
- Target modules: query, key, value
- Learning Rate: 1e-4 (ESM tolerates higher LR than BERT)
"""
...
RESULTS_FILE = RESULTS_DIR / "lora_esm2_predictions.txt"

# MODEL CONFIGURATION
MODEL_NAME = "facebook/esm2_t30_150M_UR50D"
MODEL_DISPLAY_NAME = "ESM-2 (Bi-Encoder)"


class ESM2BiEncoder(nn.Module):
    """
    Siamese Bi-Encoder architecture for PPI prediction.
    Wraps a LoRA-adapted ESM-2 model.
    """
    def __init__(self, base_model, hidden_dim=640): # ESM-2 150M has 640 dim
        super().__init__()
        self.encoder = base_model
        
        # Interaction Classifier MLP
        # Input: 4 * hidden_dim (concat, diff, product)
    ...
    def forward(self, input_ids_a, attention_mask_a, input_ids_b, attention_mask_b):
        # Encode Protein A
        # ESM-2 uses mean pooling or CLS (first token is CLS)
        out_a = self.encoder(input_ids=input_ids_a, attention_mask=attention_mask_a)
        emb_a = out_a.last_hidden_state[:, 0, :] # Use CLS token
...
def load_model_and_tokenizer(device: str = "cpu"):
    """Load ESM-2 and tokenizer with LoRA configuration."""
    ...
    # Create Bi-Encoder Model (hidden_dim=640 for 150M model)
    model = ESM2BiEncoder(encoder, hidden_dim=640)
...
def train_model(model, train_loader, val_loader, device, epochs=5, learning_rate=1e-4, verbose=True, gradient_accumulation_steps=1):
...
def main():
    ...
    print("="*80)
    print(f"MODEL 3b: {MODEL_DISPLAY_NAME}")
    print("="*80)
